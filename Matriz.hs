...
-- | Matriz datatype and operations.
--
...
module Matriz(
  ...
  , detLaplace
  , detLU
  --Mi operaciones , operadores cruzados
  -- Operadores cruzados
  , sumMatriz
  , mulMatriz
  , divEntNM
...
forceMatriz :: Matriz a -> Matriz a
forceMatriz (M n m v) = M n m $ V.map V.force $ V.force v
...
permMatriz :: Num a
           ...
           -> Int -- ^ Permuted row 1.
           -> Int -- ^ Permuted row 2.
           -> Matriz a -- ^ Permutation matrix.
permMatriz n r1 r2 | r1 == r2 = identity n
permMatriz n r1 r2 = matrix n n f
 where
  f (i,j)
...
setElem :: a -- ^ New value.
        ...
        -> Matriz a -- ^ Original matrix.
        -> Matriz a -- ^ Matriz with the given position replaced with the given value.
...
-- >                 ( 1 2 3 )
-- >                 ( 4 5 6 )   ( 1 3 )
-- > minorMatriz 2 2 ( 7 8 9 ) = ( 7 9 )
minorMatriz :: Int -- ^ Row @r@ to remove.
            -> Int -- ^ Column @c@ to remove.
            -> Matriz a -- ^ Original matrix.
            -> Matriz a -- ^ Matriz with row @r@ and column @c@ removed.
-- Requires further optimization.
minorMatriz r c (M n m v) = M (n-1) (m-1) $
...
* 'multStd':
     Matriz multiplication following directly the definition.
     This is the best choice when you know for sure that your
     matrices are small.

* 'multStrassen':
     Matriz multiplication following the Strassen's algorithm.
...
-- >               ( 1 2 3 )   (  2  4  6 )
-- >               ( 4 5 6 )   (  8 10 12 )
-- > scaleMatriz 2 ( 7 8 9 ) = ( 14 16 18 )
scaleMatriz :: Num a => a -> Matriz a -> Matriz a
scaleMatriz = fmap . (*)
...
scaleRow :: Num a => a -> Int -> Matriz a -> Matriz a
...
combineRows :: Num a => Int -> a -> Int -> Matriz a -> Matriz a
...
switchRows :: Int -- ^ Row 1.
           -> Int -- ^ Row 2.
           -> Matriz a -- ^ Original matrix.
           -> Matriz a -- ^ Matriz with rows 1 and 2 switched.
...
-- | Matriz LU decomposition with /partial pivoting/.
...
luDecomp :: (Ord a, Fractional a) => Matriz a -> (Matriz a,Matriz a,Matriz a,a)
...
-- >          ( 4 5 6 )
-- > diagProd ( 7 8 9 ) = 45
diagProd :: Num a => Matriz a -> a
diagProd = V.product . getDiag

-- DETERMINANT

-- | Matriz determinant using Laplace expansion.
--   If the elements of the 'Matriz' are instance of 'Ord' and 'Fractional'
--   consider to use 'detLU' in order to obtain better performance.
detLaplace :: Num a => Matriz a -> a
detLaplace (M 1 1 v) = V.head (V.head v)
detLaplace m =
    sum [ (-1)^(i-1) * m ! (i,1) * detLaplace (minorMatriz i 1 m) | i <- [1 .. nrows m] ]

-- | Matriz determinant using LU decomposition.
detLU :: (Ord a, Fractional a) => Matriz a -> a
